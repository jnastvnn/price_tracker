#!/usr/bin/env python3
"""Fixed upload script that properly handles boolean values in the database."""
...
def prepare_batch(listings_batch, existing_ids):
    """Prepare batch data with validation and proper type handling."""
    ...
    for listing in listings_batch:
        ...
        # FIXED: Allow False values and preserve data types
        for attr_name, attr_value in details.items():
            if attr_name and attr_value is not None:  # Changed: allow False values!
...
def bulk_insert_attributes(cursor, attributes_data, listing_id_map, attribute_cache):
    ...
    # Separate by data type for proper column usage
    attribute_relations = []
...
def main():
    ...
    print(f"🔧 FIXED UPLOAD SCRIPT - Processing {len(all_listings)} listings...")
    print("✅ Boolean values will be properly handled!")
